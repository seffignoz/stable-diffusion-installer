...
echo "############# Installing transformers (voice)"
...
############################## EXTRAS
cd ${WORKSPACE}/stable-diffusion-webui/models/Stable-diffusion && wget --content-disposition https://civitai.com/api/download/models/21877
cd ${WORKSPACE}/stable-diffusion-webui/extensions && git clone https://github.com/ahgsql/StyleSelectorXL

cd ${WORKSPACE}/stable-diffusion-webui/extensions/sd-webui-controlnet/models && git clone https://huggingface.co/lllyasviel/ControlNet-v1-1
cd ${WORKSPACE}/stable-diffusion-webui/extensions/sd-webui-controlnet/models && mv ControlNet-v1-1/* ./

# Extras PIP
echo "############# Installing extras (pip)"
sudo pip install --no-cache-dir gdown
sudo pip install --no-cache-dir clip

echo "############# Remember to start WEBUI after the reboot is done"

# Save the log before reboot
echo "############# Saving log file"
cp /var/log/cloud-init-output.log ${WORKSPACE}/install_log.log

# Sets the flag for the installation to be done
touch /home/ubuntu/installed.true
# Reboot for the Nvidia GPU to be used
echo "############# Rebooting for the Nvidia GPU to be used"
sudo reboot
